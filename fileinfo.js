...
function setupImage(tileSourceInput,tilesourceName=''){
    ...
    let tiffTileSource = new OpenSeadragon.GeoTIFFTileSource(tileSourceInput);
    
    tiffTileSource.promises.ready.then(()=>{
        document.getElementById('filename').textContent += ' -- '+tiffTileSource.GeoTIFFImages.length+' pages found';
        showTileSourcesInfo([tiffTileSource]);
    }).catch(error=>{
